...
description: Use when the user asks to operate an Android device over adb for device discovery, serial selection, package or process management, system property inspection, install or uninstall flows, app launch timing, memory or CPU or network analysis, logcat capture and filtering, UI layout dumping, screenshot or design comparison, simulated touch input, Wi-Fi or Bluetooth control, remount, root, or reboot operations on a physical device.
---

# ADB Android Ops

## Overview
Operate Android devices through a single bundled CLI instead of ad hoc adb commands. Prefer the tool when the user needs repeatable diagnostics, artifact capture, structured JSON results, or invasive device operations without pausing for confirmation.

## Quick Start
1. Confirm `adb` is available and the target device is online.
...
- `app`: inspect running state, process info, launch, stop, measure startup, or collect resource snapshots.
...
- `system`: switch adbd root mode, try remount strategies, reboot, or inspect boot completion.
